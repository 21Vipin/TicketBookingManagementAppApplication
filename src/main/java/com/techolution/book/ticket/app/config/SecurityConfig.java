package com.techolution.book.ticket.app.config;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.csrf().disable().authorizeRequests().antMatchers("/api/tickets/admin/**").hasAnyRole("admin").and().formLogin();
	}
	...
	public void configureGlobal(AuthenticationManagerBuilder auth) throws Exception{
		...
		auth.inMemoryAuthentication().withUser("raj").password("pass@123").roles("admin");
}
}
